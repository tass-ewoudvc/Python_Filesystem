...
def parse_cameras(cameras):
    ...
    next(itercams) # second line contains format, don't care for now
...
if args.city:
    citycams = filter(lambda x: args.city == x[1], cameras)
    print("Cameras in", args.city)
    printinfo(citycams)


if args.road:
    roadcams = filter(lambda x: args.road == x[2], cameras)
